...
class JudgeHF:
    """HF-transformers judge: batched bf16 generation + JSON score parse."""

    def __init__(self, name: str, model_id: str):
        self.name = name
        self.model_id = model_id
        self.tok = AutoTokenizer.from_pretrained(model_id, cache_dir="/workspace/hf_cache", token=os.environ.get("HF_TOKEN"))
    ...
    def score(self, propositions, paragraphs, batch_size: int = 16, max_new_tokens: int = 180):
        prompts = []
        for prop, para in zip(propositions, paragraphs):
            user_msg = CLARITY_RUBRIC.format(proposition=prop, paragraph=para or "")
            chat = [
                {"role": "system", "content": JUDGE_SYSTEM},
                {"role": "user", "content": user_msg},
            ]
            prompts.append(self.tok.apply_chat_template(chat, tokenize=False, add_generation_prompt=True))

        scores = []
        for i in range(0, len(prompts), batch_size):
            b = prompts[i:i + batch_size]
            enc = self.tok(b, return_tensors="pt", padding=True, truncation=True, max_length=2048).to("cuda")
...
def main():
    ...
    run = wandb.init(
        ...
        name=f"grpo-min-repro-{time.strftime('%Y%m%d-%H%M%S')}",
    ...
    manifest = capture_manifest(
        ...
        extra={
            "purpose": "exact_repro_of_grpo-min-20260417-144201",
            "rewriter": REWRITER,
            "judges_train": [m for _, m in JUDGE_ENSEMBLE],
            "judge_heldout": HELDOUT[1],
            "judge_backend": "HF transformers (JudgeHF class)",
            "rewriter_backend": "HF generate (no vLLM)",
