...
under the run's output dir (e.g. `/data/shil6647/attack-llm-judge/grpo_run/
pilot_<suffix>/STOP`) so it is per-run and survives restarts. Each script
clears any stale STOP on launch so a marker from a previous aborted run
doesn't fire immediately.

Resume contract:
- GRPO: re-run the same command with `--resume-from <ckpt>`; pass
  `--skip-pre-eval` to skip the 14-min pre-eval if a `pre_eval.json` was
  already persisted.
...
def clear_stop_file(stop_file: str | os.PathLike) -> None:
    ...
    Without this, a STOP from a previous (aborted) run would fire on the
    very first step/batch of the new run.
    """
    p = Path(stop_file)
    if p.exists():
        p.unlink()
...
def build_trainer_callback(stop_file: str):
    """Return a HF TrainerCallback that triggers save + stop on STOP-file presence.

    Imported lazily because transformers is only a dep for the training script.
    """
